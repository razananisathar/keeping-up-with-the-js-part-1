...
const setLoggedInUser= (user) => {
    ...
    const  elCurrentUser = `Logged in as <span id="loggedInUser">${user.fname}&nbsp;${user.lname}</span>`;
    elPara.innerHTML = elCurrentUser;
};
...
const decrypt = (ctxt) => {
    const dectxt = CryptoJS.AES.decrypt(ctxt, "rabbit");
...
function addTodo(e) {
    ...
    if(elTodo.value !== "" && index !== -1) {
        //clear if error exists.
        setInputError("", elTodo);
...
function loadToDoLists(lists) {
    ...
    if(lists.length === 0) {
        ...
        elUL.innerHTML = `${revlists.map((item, i) => 
                            `<li id="list${i}">${item.name}</li>`).join("")}`;
